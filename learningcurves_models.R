...
binme<-function(somedat,elogit=TRUE){	
	...
	binned_non<-binned_s
	for (y in 4:30){
		assign(paste("binned_f",y,sep=""),cbind(smooth2(somedat[somedat$Eng_little=="little" & somedat$te==y,c("t","elogit")],2,0),AoFE=y))
	}
	...
	for (y in seq(1,30,jump)){
		assign(paste("binned_i",y,sep=""),cbind(smooth2(somedat[somedat$Eng_little=="lot" & somedat$te>=y & somedat$te<(y+jump),c("t","elogit")],2,0),AoFE=y+(jump-1)/2))
	}
	...
	somebins<-rbind(binned_m,binned_s,binned_imm,binned_non)
	...
	somebins$E<-1
	somebins$N<-1 #only needed for the new R2 versions. Don't use if you want raw R2.
	return(somebins)
}
...
DEshmccv<-function(n,bptype){
	...
	for (i in 1:n){
		...
		PAR<-c(DEoptim(fitfunc,lower=lowers,upper=uppers,DEoptim.control(VTR=.01,trace=TRUE,itermax=500,parallelType=2,parVar=c("tf","basefunc")))[[1]]$bestmem)
...
plotdat<-function(logodds=TRUE){
	...
	if(logodds){
		load("../../Analyses_final/data_binned")
	}else{
		load("../../Analyses_final/data_binned_per")
	}
	toplot<-binned
	toplot$te<-as.factor(toplot$te)
	toplot$fac<-as.factor(paste(toplot$Eng_little,toplot$te,sep=""))
	...
	pdf(file="learningcurves.pdg",w=7,h=7)
	vplayout <- function(x,y) viewport(layout.pos.row=x, layout.pos.col=y)
	pushViewport(viewport(layout=grid.layout(2,2,heights=c(1,1),widths=c(.5,.5))))
	print(p,vp=vplayout(1,1))
	print(q,vp=vplayout(2,1))
