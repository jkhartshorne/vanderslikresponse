...
#loss_curve_flat <- function(r,a,d,Eb,Ei,En,tc,te,t,group){ # FRANS Ei eruit
loss_curve_flat <- function(r,a,d,Eb,tc,te,t,group){
  E <- #ifelse(#group == "non-immersion",
#              En,
#              ifelse(group == "immersion", # FRANS immersion eruit
#                    Ei)
...
loss_curve_piecewise_bi <- function(r,a1,a2,d1,d2,Eb,te,t,group){    
  E <- #ifelse(group == "non-immersion",
#              En,
...
loss_curve_piecewise_asymptote_bi <- function(r,a1,a2,d1,d2,e,te,t,group){
  E <- #ifelse(group == "non-immersion",
#              En,
#              ifelse(group == "immersion",
#                     Ei,
...
predicted_curve_piecewise_bi <- function(r,a1,a2,d1,d2,E,te,t){
  ta <- (a1*d1- a2*d2)/(a1-a2)
  ifelse(t <= ta,
         -E*r*((t-te)+1/a1*log((1+exp(a1*(te-d1)))/(1+exp(a1*(t-d1))))),
         ifelse(te < ta,
...
prediction_piecewise_bi <- function(r,a1,a2,d1,d2,E,te,t){
  (1 - exp(predicted_curve_piecewise(r,a1,a2,d1,d2,E,te,t)))*2 + 1.5
}

prediction_piecewise_irt_bi <- function(r,a1,a2,d1,d2,E,te,t){
  (1 - exp(predicted_curve_piecewise(r,a1,a2,d1,d2,E,te,t)))*2 - 1.5
}

predicted_curve_flat_bi <- function(r,a,d,E,tc,te,t){
  ifelse(t <= tc,
         -E*r*(t-te),
         ifelse(t > tc & te <= tc,
                -E*r*((tc-te) + (t-tc) + (1/a*log((1+exp(-a*d))/(1+exp(a*(t-tc-d)))))),
...
prediction_flat_bi <- function(r,a,d,E,tc,te,t){
  (1- exp(predicted_curve_flat(r,a,d,E,tc,te,t)))*2 + 1.5
}

prediction_flat_irt_bi <- function(r,a,d,E,tc,te,t){
  (1- exp(predicted_curve_flat(r,a,d,E,tc,te,t)))*2 - 1.5
}