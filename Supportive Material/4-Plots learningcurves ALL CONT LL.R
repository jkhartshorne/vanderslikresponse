...
#
# Generates rate plot for continuous model
#

library("grid")
library(ggplot2)

PAR <- c(1.0, .16,  .24,  30.80)  # Parameters of Continuous ELSD 
logodds = T
  
E <- c(.70, .80, .24) #  bileng, lot, little CONTINUOUS 
...
r 
pdf(file=paste("name","all_ALL CONT",".pdf",sep="_"),w=8,h=8)
...
pdf(file=paste("name","main_ALL_CONT",".pdf",sep="_"),w=7,h=7)
vplayout <- function(x,y) viewport(layout.pos.row=x, layout.pos.col=y)
...
pdf(file=paste("name","rate_CONT",".pdf",sep="_"),w=5,h=3.5)
print(r)
dev.off()