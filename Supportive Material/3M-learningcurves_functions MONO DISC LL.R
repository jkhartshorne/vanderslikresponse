...
loss_curve_flat <- function(r,a,d,tc,te,t,group){
...
loss_flat_elogit_mo <-function(par){
#  sum(((1-exp(loss_curve_flat(r = par[1], a = par[2], d = par[3], Eb = par[4], Ei = par[5], En = par[6], tc = par[7],
   sum(((1-exp(loss_curve_flat(r = par[1], a = par[2], d = par[3], tc = par[4],
   te = tofit$te, t = tofit$age, group = tofit$groups)))*2 + 1.5 - tofit$ability)^2)
}

loss_flat_irt_mo <-function(par){
#  sum(((1-exp(loss_curve_flat(r = par[1], a = par[2],d = par[3],Eb = par[4], Ei = par[5], En = par[6], tc = par[7],
  sum(((1-exp(loss_curve_flat(r = par[1], a = par[2],d = par[3], tc = par[4], te = tofit$te, t = tofit$age, group = tofit$groups)))*2 - 1.5 - tofit$ability)^2)
}
...
loss_curve_piecewise_mo <- function(r,a1,a2,d1,d2,te,t,group){
  E <- #ifelse(group == "non-immersion",
#              En,
#              ifelse(group == "immersion",
#                     Ei,
...
loss_curve_piecewise_asymptote_mo <- function(r,a1,a2,d1,d2,e,te,t,group){
  E <- #ifelse(group == "non-immersion",
#              En,
#              ifelse(group == "immersion",
#                     Ei,
...
loss_piecewise_asymptote_mo <- function(par){
...
predicted_curve_piecewise_mo <- function(r,a1,a2,d1,d2,E,te,t){
  ta <- (a1*d1- a2*d2)/(a1-a2)
  ifelse(t <= ta,
         -E*r*((t-te)+1/a1*log((1+exp(a1*(te-d1)))/(1+exp(a1*(t-d1))))),
         ifelse(te < ta,
...
prediction_piecewise_mo <- function(r,a1,a2,d1,d2,E,te,t){
  (1 - exp(predicted_curve_piecewise(r,a1,a2,d1,d2,E,te,t)))*2 + 1.5
}

prediction_piecewise_irt_mo <- function(r,a1,a2,d1,d2,E,te,t){
  (1 - exp(predicted_curve_piecewise(r,a1,a2,d1,d2,E,te,t)))*2 - 1.5
}

predicted_curve_flat_mo <- function(r,a,d,E,tc,te,t){
  ifelse(t <= tc,
         -E*r*(t-te),
         ifelse(t > tc & te <= tc,
                -E*r*((tc-te) + (t-tc) + (1/a*log((1+exp(-a*d))/(1+exp(a*(t-tc-d)))))),
...
prediction_flat_mo <- function(r,a,d,E,tc,te,t){
  (1- exp(predicted_curve_flat(r,a,d,E,tc,te,t)))*2 + 1.5
}

prediction_flat_irt_mo <- function(r,a,d,E,tc,te,t){
  (1- exp(predicted_curve_flat(r,a,d,E,tc,te,t)))*2 - 1.5
}